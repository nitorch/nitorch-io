...
from . import py, dtypes, torchutils as utils
import torch
...
def cutoff(dat, cutoff, dim=None):
    ...
    if torch.is_tensor(dat):
        ...
        cutoff = utils.as_tensor([val/100 for val in cutoff],
                                 dtype=torch.get_default_dtype(),
                                 device=dat_for_quantile.device)
        pct = torch.quantile(dat_for_quantile, cutoff)
